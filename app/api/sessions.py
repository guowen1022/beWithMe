...
async def session_graph(
    ...
    if label:
        # PostgreSQL array contains: labels @> ARRAY['NLP']
    ...
    # Get duration for each session (time from first to last interaction)
    ...
    for s in summaries:
        ...
        # Labels: prefer DB column (mutable), fall back to file parse, then ["General"]
