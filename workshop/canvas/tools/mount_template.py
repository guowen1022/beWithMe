...
def build_spec(user_id: UUID) -> ToolSpec:
    return ToolSpec(
        ...
        description=(
            ...
            "lists, ` ```mermaid ` for flow/process diagrams, and "
            "` ```plot ` for **coordinate charts with real axes** "
            "(2D curves, 3D surfaces). Plot body is JSON: "
            "`{\"mode\":\"2d\",\"expression\":\"x*x\",\"x_range\":[-3,3]}` "
            "or `{\"mode\":\"3d_surface\",\"expression\":\"x*x+y*y\","
            "\"path\":[{\"x\":2,\"y\":2},{\"x\":0,\"y\":0}]}`. "
            "Use plot (not mermaid) for regression lines, loss surfaces, "
            "function graphs, scatter plots — anything with numeric axes. "
            "Don't author raw `<div class=\"card-…\">` — "
            ...
            "and numbered lists, ` ```mermaid ` for flow diagrams, "
            "` ```plot ` for coordinate charts (JSON config, see above). "
            "Inline HTML is allowed for special cases "
