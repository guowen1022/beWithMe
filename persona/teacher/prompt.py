"""Teacher prompt construction — builds the LLM prompt from brain state.

This module owns the teacher agent's "voice": how it interprets the
silicon brain's data and structures it into a prompt for the LLM.

Hint-only imports from silicon_brain live in a TYPE_CHECKING block so
persona takes no runtime dependency on silicon_brain for type shape —
only the type checker resolves them. Annotations are stringified via
`from __future__ import annotations` so duck-typed callers (and future
DTOs from an HTTP-driven persona) work without runtime errors.
"""
from __future__ import annotations

import re
from typing import Optional, List, NamedTuple, Tuple, TYPE_CHECKING
