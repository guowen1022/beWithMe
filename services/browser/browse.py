"""User-facing browse routes — status, selection, handoff, render, resume.

The public face of the browser sidecar (plus the internal `/render` used by the
knowledge sidecar to fulfil `/documents/url`). These drive the *visible* page
(`browse_page` / `handoff_page`) on `app.state`, distinct from the headless
`browser_set` session (`session.py`) and the Electron `web_view`
(`web_view_routes.py`). Extracted verbatim from `main.py` (F6).
"""
